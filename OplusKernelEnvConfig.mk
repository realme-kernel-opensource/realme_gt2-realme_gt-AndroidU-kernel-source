...
subsystem_sleep_monitor_mk_path := $(abspath $(lastword $(MAKEFILE_LIST)))
subsystem_sleep_monitor_mk_dir := $(shell dirname $(subsystem_sleep_monitor_mk_path))
subsystem_sleep_monitor_source_dir := $(subsystem_sleep_monitor_mk_dir)/../vendor/oplus/kernel/power/subsys_sleep_monitor
subsystem_sleep_monitor_target_dir := $(subsystem_sleep_monitor_mk_dir)/../kernel/msm-5.4/drivers/soc/oplus/power/subsys_sleep_monitor
power_dir := $(subsystem_sleep_monitor_mk_dir)/../kernel/msm-5.4/drivers/soc/oplus/power
power_dir_exist := $(shell if [ -d $(power_dir) ]; then echo "exist"; else echo "notexist"; fi;)
ifeq ($(power_dir_exist), notexist)
$(shell mkdir -p $(power_dir))
endif
subsystem_sleep_monitor_source_dir_exist := $(shell if [ -d $(subsystem_sleep_monitor_source_dir) ]; then echo "exist"; else echo "notexist"; fi;)
subsystem_sleep_monitor_target_dir_exist := $(shell if [ -d $(subsystem_sleep_monitor_target_dir) ]; then echo "exist"; else echo "notexist"; fi;)
ifeq ($(subsystem_sleep_monitor_source_dir_exist)_$(subsystem_sleep_monitor_target_dir_exist), exist_notexist)
$(warning "source is $(subsystem_sleep_monitor_source_dir)")
$(shell ln -sf $(subsystem_sleep_monitor_source_dir) $(subsystem_sleep_monitor_target_dir))
endif
